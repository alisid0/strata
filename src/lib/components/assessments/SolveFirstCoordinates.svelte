...
<div class="solve-first">
  ...
  {#key phase}
    <section class="phase" in:fly={{ x: 28, duration: 240 }} out:fade={{ duration: 90 }}>
      ...
      {#if phase === 'brief'}
        <div class="brief">
          <div class="cart-mark" aria-hidden="true"><span class="ping"></span></div>
          <div class="micro-label">Underground rescue · Location layer down</div>
          <h2>Find the Signal</h2>
          <p>A storm knocked out the map. Four teams still have live signal beacons, but their positions are gone. You have one locator drone.</p>
          <div class="mission">
          ...
          <button class="primary" on:click={() => phase = 'oneline'}>Power the locator</button>
        </div>
...
<style>
  ...
  .cart-mark .ping { width: 18px; height: 18px; border-radius: 50%; background: var(--qx-accent); box-shadow: 0 0 0 8px var(--qx-accent-soft), 0 0 0 16px var(--qx-accent-soft-2); }
