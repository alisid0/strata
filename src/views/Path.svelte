...
<style>
  ...
  .topic-tile:not(.future):hover { border-color: var(--qx-accent); transform: translateY(-1px); }
  .tile-icon-wrap {
    width: 34px; height: 34px; border-radius: 8px; background: var(--qx-surface-2);
    display: flex; align-items: center; justify-content: center; flex-shrink: 0;
  }
  .tile-icon { width: 26px; height: 26px; object-fit: contain; display: block; }
  .tile-name {
    min-height: 35px;
    font-size: 14px; font-weight: 800; color: var(--qx-text);
    line-height: 1.22; overflow-wrap: anywhere;
  }
  .tile-meta { font-size: 11.5px; font-weight: 650; color: var(--qx-text-faint); }
  .tile-chip {
    margin-top: auto;
  ...
  .tile-chip.started { color: var(--qx-accent-text); background: var(--qx-accent-soft); }
  .tile-chip.done { color: var(--qx-green-text); background: var(--qx-green-soft); }
  .row-chev { display: none; }

  .topic-tile.future { cursor: default; opacity: 0.62; background: transparent; border-style: dashed; }
